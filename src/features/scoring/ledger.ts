 ...
 *
 * writeLedgerForMatch: takes a match_id whose result is confirmed
 * (or admin_set), looks up the match + result + tournament + player
 * tiers, runs calculate(), and inserts one points_ledger row per
 * player. Atomic via db.transaction. Idempotent via the
 * uq_player_match unique constraint + onConflictDoNothing.
 *
 * rewriteLedgerForMatch: deletes existing ledger rows for the match
 * then re-writes. Used by admin override paths in M3 Task 5.6.
 */
...
export async function rewriteLedgerForMatch(matchId: string): Promise<{
  ...
  return db.transaction(async (tx) => {
    ...
    // Re-call writeLedgerForMatch outside this transaction so the delete
    // commits first; writeLedgerForMatch starts its own transaction.
    // (Drizzle's postgres-js doesn't nest, so we close this one early.)
    return { inserted: 0 };
  }).then(async () => {
