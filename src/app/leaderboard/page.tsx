...
export default async function LeaderboardPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 pt-10 pb-24">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-rule)] pb-3 text-[10px] uppercase tracking-[0.22em] text-[var(--color-fg-muted)]">
        <span>§ Leaderboard</span>
        <span>All tiers · Current month</span>
      </header>

      <div className="mt-16 flex flex-wrap items-baseline justify-between gap-4">
        <h1 className="text-5xl md:text-7xl font-light leading-[0.9] tracking-tight">
          Leader<span className="text-[var(--color-pink)]">board</span>
        </h1>
        <span className="text-[10px] uppercase tracking-[0.22em] text-[var(--color-fg-muted)] tabular-nums">
          {dbError ? '—' : `${String(rows.length).padStart(2, '0')} player${rows.length === 1 ? '' : 's'}`}
        </span>
      </div>

      <p className="mt-6 max-w-2xl text-sm md:text-base text-[var(--color-fg-muted)] leading-relaxed">
        Players ranked by cumulative points within their tier. Weeks at rank one
        accumulate toward auto-promotion. Snapshots run nightly. Ties broken by
        match count, then registration order.
      </p>
        ...
        {dbError ? (
          ...
          <div className="border-t border-[var(--color-rule)]">
            <table className="w-full text-sm md:text-base">
              <thead>
                <tr className="text-left text-[10px] uppercase tracking-[0.22em] text-[var(--color-fg-muted)]">
              ...
              <tbody>
                {rows.map((p, i) => (
                  <tr
                    key={p.handle}
                    className="border-t border-[var(--color-rule)] hover:bg-white/[0.02] transition-colors"
                  >
                    <td className="py-4 pr-4 text-[var(--color-fg-muted)] tabular-nums">
                      {String(i + 1).padStart(2, '0')}
                    </td>
                    <td className="py-4 pr-4 text-[var(--color-fg)]">
                      {p.handle}
                    </td>
...
function EmptyState({
  ...
  return (
    <div className="border border-dashed border-[var(--color-rule)] px-6 md:px-10 py-16 text-center">
      <p className="text-[10px] uppercase tracking-[0.22em] text-[var(--color-pink)] mb-5">
        No data
      </p>
      ...
      {code ? (
        <pre className="mt-8 inline-block max-w-full overflow-x-auto border-l-2 border-[var(--color-pink-dim)] pl-3 text-left text-[11px] text-[var(--color-fg-muted)]">
          <code>{code}</code>
        </pre>
      ) : null}
    </div>
